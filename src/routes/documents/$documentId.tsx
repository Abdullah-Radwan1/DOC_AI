...
const severityColors = {
  high: { bg: 'bg-red-500/10', text: 'text-red-500', border: 'border-red-500/30', badge: 'destructive' as const },
  medium: { bg: 'bg-amber-500/10', text: 'text-amber-500', border: 'border-amber-500/30', badge: 'secondary' as const },
  low: { bg: 'bg-blue-500/10', text: 'text-blue-500', border: 'border-blue-500/30', badge: 'outline' as const },
};
...
export function DocumentPage() {
  ...
  const { data: analysis, isLoading: analysisLoading } = useDocumentAnalysis(documentId);
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        <div className="space-y-2">
          <Link to="/documents" className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-colors mb-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">{document?.filename || 'Contract_Acme_2024.pdf'}</h1>
              <p className="text-muted-foreground text-sm">
                Uploaded on {new Date(document?.created_at || Date.now()).toLocaleDateString()} by Demo User
              </p>
            </div>
          </div>
        </div>
      ...
      <motion.div variants={itemVariants} className="grid gap-4 md:grid-cols-4">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-lg ${
                complianceScore >= 80 ? 'bg-green-500/10' :
                complianceScore >= 60 ? 'bg-amber-500/10' : 'bg-red-500/10'
              }`}>
                <Shield className={`h-5 w-5 ${
                  complianceScore >= 80 ? 'text-green-500' :
                  complianceScore >= 60 ? 'text-amber-500' : 'text-red-500'
                }`} />
              </div>
              <div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{((analysis?.risks as Record<string, any>)?.risks as any[])?.length || 6}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-lg bg-amber-500/10">
                <AlertCircle className="h-0 h-5 w-5 text-amber-500" />
              </div>
              <div>
                <p className="text-2xl font-bold">{(analysis?.missing_clauses as any)?.clauses?.length || 3}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{(analysis?.important_dates as any)?.dates?.length || 5}</p>
      ...
      <motion.div variants={itemVariants}>
        <Card className="bg-gradient-to-br from-blue-500/5 via-background to-cyan-500/5 border-blue-500/20">
          <CardHeader>
            <div className="flex items-center gap-2">
      ...
      <motion.div variants={itemVariants}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Detailed breakdown of contract terms and parties</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <div className="space-y-3">
                  {((analysis?.obligations as any)?.obligations || [
                    ...
                    <div key={i} className="flex items-start gap-3 p-3 rounded-lg border border-border/50">
                      <div className={`p-2 rounded-lg ${i < 2 ? 'bg-blue-500/10' : 'bg-cyan-500/10'}`}>
                        <Users className={`h-4 w-4 ${i < 2 ? 'text-blue-500' : 'text-cyan-500'}`} />
                      </div>
                      <div>
              ...
              <TabsContent value="penalties" className="mt-4">
                <div className="space-y-3">
                  {((analysis?.penalties as any)?.penalties || [
                    ...
                    <div key={i} className="flex items-start gap-3 p-4 rounded-lg border border-red-500/30 bg-red-500/5">
                      <div className="p-2 rounded-lg bg-red-500/10">
                        <Gavel className="h-4 w-4 text-red-500" />
                      </div>
                      <div>
                        <p className="text-sm font-medium">{item.type}</p>
                        <p className="text-sm text-muted-foreground">{item.penalty}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="p-4 rounded-lg border border-border/50">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {analysis?.governing_law || 'State of Delaware, United States'}
                    </p>
                  </div>
                  <div className="p-4 rounded-lg border border-border/50">
                    <div className="flex items-center gap-3 mb-3">
                    ...
                    {((analysis?.renewal_terms as any)?.terms || [
                      { type: 'Auto-renewal', period: '12 months', notice_period: '60 days prior' },
                    ]).map((term: any, i: number) => (
                      <div key={i} className="text-sm text-muted-foreground">
                        <p>{term.type}: {term.period}</p>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1 space-y-2">
                <p className="font-medium">
                  {complianceScore >= 80 ? 'Good Standing' : complianceScore >= 60 ? 'Needs Attention' : 'Critical Issues'}
                </p>
                <p className="text-sm text-muted-foreground">
                  {complianceScore >= 80
            ...
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {((analysis?.compliance_requirements as any)?.requirements || [
                  { regulation: 'GDPR', status: 'Partially Compliant', details: 'Data processing clauses present' },
                  { regulation: 'SOC 2', status: 'Compliant', details: 'SOC 2 Type II required annually' },
                  { regulation: 'HIPAA', status: 'Not Applicable', details: 'No PHI data involved' },
                  { regulation: 'CCPA', status: 'Partially Compliant', details: 'California notification required' },
                ]).map((req: any, i: number) => (
                  <div key={i} className="flex items-start gap-3 p-3 rounded-lg border border-border/50">
                    <div className={`w-3 h-3 rounded-full mt-1 ${
                      req.status === 'Compliant' ? 'bg-green-500' :
                      req.status === 'Not Applicable' ? 'bg-slate-400' : 'bg-amber-500'
                    }`} />
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium">{req.regulation}</p>
                        <Badge variant={req.status === 'Compliant' ? 'default' : 'secondary'} className="text-xs">
                          {req.status}
                        </Badge>
                      </div>
          ...
          <CardContent>
            <div className="grid md:grid-cols-3 gap-4">
              {['high', 'medium', 'low'].map((severity) => (
                <div key={severity} className={`rounded-lg ${severityColors[severity as keyof typeof severityColors].bg} border ${severityColors[severity as keyof typeof severityColors].border} p-4`}>
                  <div className="flex items-center gap-2 mb-4">
                    <Badge variant={severityColors[severity as keyof typeof severityColors].badge} className="capitalize">
                      {severity} Risk
                    </Badge>
                    <span className={`text-sm ${severityColors[severity as keyof typeof severityColors].text}`}>
                      {((analysis?.risks as any)?.risks || []).filter((r: any) => r.severity === severity).length ||
                        (severity === 'high' ? 2 : severity === 'medium' ? 3 : 1)}
                    </span>
                  </div>
                  <div className="space-y-3">
                    {((analysis?.risks as any)?.risks || [
                      { category: 'Legal', description: 'Indemnification clause absent', severity: 'high' },
                      { category: 'Financial', description: 'Unlimited data breach liability', severity: 'high' },
                      { category: 'Operational', description: 'Auto-renewal limited notice', severity: 'medium' },
                      { category: 'Compliance', description: 'GDPR DPA not attached', severity: 'medium' },
                      { category: 'Legal', description: 'Force majeure missing', severity: 'medium' },
                      { category: 'Operational', description: 'Short audit notice period', severity: 'low' },
                    ])
          ...
          <CardContent>
            <div className="space-y-3">
              {((analysis?.missing_clauses as any)?.clauses || [
                ...
                { name: 'Indemnification', severity: 'high', recommendation: 'Include mutual indemnification' },
                { name: 'Intellectual Property Rights', severity: 'low', recommendation: 'Clarify IP ownership' },
              ]).map((clause: any, i: number) => (
                <div key={i} className={`flex items-start gap-3 p-3 rounded-lg border ${severityColors[clause.severity as keyof typeof severityColors].border}`}>
                  <div className={`p-2 rounded-lg ${severityColors[clause.severity as keyof typeof severityColors].bg}`}>
                    <AlertCircle className={`h-4 w-4 ${severityColors[clause.severity as keyof typeof severityColors].text}`} />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <p className="font-medium">{clause.name}</p>
                      <Badge variant={severityColors[clause.severity as keyof typeof severityColors].badge}>
                        {clause.severity}
                      </Badge>
                    </div>
                    <p className="text-sm text-muted-foreground mt-1">{clause.recommendation}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <motion.div variants={itemVariants}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Prioritized action items based on risk assessment</CardDescription>
          </CardHeader>
          <CardContent>
            <Accordion type="multiple" className="space-y-2">
              {((analysis?.recommendations as any)?.recommendations || [
                ...
                <AccordionItem key={i} value={`rec-${i}`} className="border rounded-lg px-4">
                  <AccordionTrigger className="hover:no-underline">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge variant="outline" className="capitalize hidden sm:inline-flex">{rec.priority}</Badge>
                    </div>
                  </AccordionTrigger>
                  <AccordionContent>
                    <p className="text-sm text-muted-foreground pl-5">{rec.description}</p>
                  </AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
            ...
            <div className="relative">
              ...
              <div className="space-y-6">
                {((analysis?.important_dates as any)?.dates || [
                  ...
                  { type: 'Expiration Date', date: '2025-01-14', description: 'Initial term expires' },
                  ...
                  { type: 'SOC 2 Report Due', date: '2024-03-01', description: 'Annual certification required' },
                ]).map((item: any, i: number) => (
                  <div key={i} className="relative pl-10">
                    <div className="absolute left-2 top-1 w-4 h-4 rounded-full bg-background border-2 border-blue-500" />
                    <div className="p-4 rounded-lg border border-border/50">
      ...
      {/* Footer Actions */}
      <motion.div variants={itemVariants} className="flex items-center justify-between py-4 border-t">
